...
def server(port):
    ...
    while True:
        ...
        print(f'Connection from {sockname}')
        print(f' Socket name: {sc.getsockname()}')
        print(f' Socket peer: {sc.getpeername()}')
        message = recvall(sc, 16)
        print(f' message from client: {repr(message)}')
...
def client(port):
    ...
    print(f'Client has been assigned the socket: {sock.getsockname()}')
    sock.sendall(b'Greetings, server')
    reply = recvall(sock, 16)
    print(f'Server: {repr(reply)}')
    sock.close()
